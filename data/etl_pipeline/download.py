    ...
    def download_file(
        ...
        for attempt in range(self.download_config.retry.max_retries + 1):
            ...
            try:
                ...
                last_error = str(e)
                self.logger.warning(f"Download attempt {attempt + 1} failed for {source.name}: {e}")
    ...
    def _download_with_progress(
        ...
        if source.checksum:
            if not self.verify_checksum(dest_path, source.checksum):
                ...
                raise ChecksumError(
                    f"Checksum mismatch for {source.name}. " f"Expected: {source.checksum}"
                )
            checksum_verified = True

        duration = time.time() - start_time
        self.logger.info(f"Downloaded {source.name}: {bytes_downloaded:,} bytes in {duration:.1f}s")
    ...
    def download_batch(
        ...
        if max_parallel == 1:
            ...
            with ThreadPoolExecutor(max_workers=max_parallel) as executor:
                future_to_source = {
                    executor.submit(self.download_file, source): source for source in sources
                }
