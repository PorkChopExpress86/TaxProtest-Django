...
class FieldSchema:
    ...
    def get_source_name(self, available_fields: list[str]) -> str | None:
...
class DataTransformer:
    ...
    def transform_file(
        ...
        schema: TableSchema,
        limit: int | None = None,
    ) -> Generator[tuple[dict[str, Any] | None, list[ValidationError]], None, TransformResult]:
...
EXTRA_FEATURES_SCHEMA = TableSchema(
    name="extra_features",
    fields=[
        # Fallback columns: acct, bld_num, count, grade, cd, s_dscr, l_dscr, cat, dscr, note, uts
        # Detail columns: acct, cd, dscr, grade, cond_cd, bld_num, length, width, units, act_yr, asd_val
        FieldSchema("account_number", ["acct"], "str", max_length=20, required=True),
        FieldSchema("feature_number", ["bld_num"], "int"),
        FieldSchema("feature_code", ["cd"], "str", max_length=10),
        FieldSchema("feature_description", ["l_dscr", "dscr"], "str", max_length=255),
        FieldSchema("quantity", ["count", "units"], "decimal"),
        ...
        FieldSchema("year_built", ["act_yr"], "int"),
        FieldSchema("value", ["uts", "asd_val"], "decimal"),
    ],
...
def get_schema(name: str) -> TableSchema | None:
