...
class ModelLoader:
            ...
            with transaction.atomic():
                for record in records:
                    ...
                    feature = ExtraFeature(
                        ...
                        feature_code=self._safe_str(record.get("feature_code"), max_length=10),
                        feature_description=self._safe_str(
                            record.get("feature_description"), max_length=255
                        ),
                        ...
                        quality_code=self._safe_str(record.get("quality_code"), max_length=10),
                        condition_code=self._safe_str(record.get("condition_code"), max_length=10),
                        year_built=self._safe_int(record.get("year_built")),
    ...
    def _safe_str(value: Any, *, max_length: int) -> str:
        """Safely convert optional source values to model string fields."""
        if value is None:
            return ""
        return str(value).strip()[:max_length]

    def _get_bedrooms(
        self, account_num: str, building_num: int, record: dict[str, Any]
    ) -> int | None:
        """
