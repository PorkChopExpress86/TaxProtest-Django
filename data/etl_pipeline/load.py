...
class LoadManager:
    ...
    def load_with_fk(
        ...
        fk_cache = {getattr(obj, fk_lookup): obj for obj in fk_model.objects.all().only(fk_lookup)}
    ...
    def create_checkpoint(
        ...
        self.logger.debug(f"Checkpoint: {import_id} - {records_processed:,} records")
        return checkpoint

    def get_checkpoint(self, import_id: str) -> ImportCheckpoint | None:
    ...
    def delete_inactive(
        ...
        while True:
            ...
            ids = list(
                model_class.objects.filter(is_active=False).values_list("pk", flat=True)[
                    :batch_size
                ]
            )
        ...
        self.logger.info(f"Deleted {total_deleted:,} inactive {model_class.__name__} records")
