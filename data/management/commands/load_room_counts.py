...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Total fixture records: {results["total_fixture_records"]:,}')
            )
            self.stdout.write(
                self.style.SUCCESS(f'Room records (RMB/RMF/RMH): {results["room_records_found"]:,}')
            )
            self.stdout.write(
                self.style.SUCCESS(
                    f'BuildingDetail records updated: {results["buildings_updated"]:,}'
                )
            )

            if results["buildings_not_found"] > 0:
                self.stdout.write(
                    self.style.WARNING(f'Buildings not found: {results["buildings_not_found"]:,}')
                )
