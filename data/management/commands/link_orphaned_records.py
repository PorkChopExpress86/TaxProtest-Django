...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Buildings invalid: {results["buildings_invalid"]}')
            )
            self.stdout.write(self.style.SUCCESS(f'Features linked: {results["features_linked"]}'))
            self.stdout.write(
                self.style.SUCCESS(f'Features invalid: {results["features_invalid"]}')
            )
            ...
            if results["buildings_invalid"] > 0 or results["features_invalid"] > 0:
                ...
                self.stdout.write(
                    self.style.WARNING("These may need to be investigated or cleaned up manually.")
                )
