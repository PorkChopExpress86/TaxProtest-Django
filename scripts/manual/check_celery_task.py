...
Usage:
    docker compose exec web python scripts/manual/check_celery_task.py
"""
...
from counties.harris.tasks_new import download_and_import_building_data
...
def main():
    ...
    try:
        ...
        print('  docker compose exec web python -c "')
        print("from celery.result import AsyncResult")
        print(f"t = AsyncResult('{task.id}')")
        print("print(f'State: {t.state}')")
        print("print(f'Result: {t.result}')\"")
        print("=" * 70)
