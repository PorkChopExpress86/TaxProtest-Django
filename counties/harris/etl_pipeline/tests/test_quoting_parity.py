"""Quote-handling contract for HCAD readers.

The shared reader owns source parsing for COPY and ORM persistence. It and
the generic transformer retain embedded quotes literally via ``QUOTE_NONE``.
"""
...
class QuotingContractTests(SimpleTestCase):
    """Shared and generic readers must handle embedded quotes identically."""
    ...
    def test_embedded_quote_does_not_corrupt_rows(self) -> None:
        ...
        shared_reader, shared_fh = _open_text(path)
        shared_rows = list(shared_reader)
        shared_fh.close()
        ...
        self.assertEqual(shared_rows[2][0], "R2")
        self.assertEqual(shared_rows[2][2], 'OAK "AVE"')
        self.assertEqual(shared_rows[2][3], '200 OAK "AVE"')
        ...
        self.assertEqual(shared_rows[3][0], "R3")
