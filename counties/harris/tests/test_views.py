...
class SimilarPropertiesViewTests(TestCase):
    ...
    def test_similar_results_sorted_by_similarity_score(self, mock_find_similar):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.context["subject"].key, self.target.account_number)
        comps = response.context["comps"]
        self.assertEqual(comps[0].key, self.high_ppsf_property.account_number)
        self.assertEqual(comps[0].match_label, "Best match")
        self.assertEqual(comps[1].key, self.low_ppsf_property.account_number)
    ...
    def test_similar_properties_includes_assessment_history(self, mock_find_similar):
        ...
        self.assertContains(response, "Assessment History")
    ...
    def test_similar_properties_hides_history_when_absent(self, mock_find_similar):
        ...
        self.assertNotContains(response, "Assessed Value Trend")
...
class ProtestRecommendationTests(TestCase):
    ...
    def test_strong_protest_recommendation(self, mock_find_similar):
        ...
        self.assertIn("Recommend protesting", recommendation.headline)
        self.assertIsNotNone(recommendation.median)
        self.assertEqual(recommendation.comparable_count, 3)
    ...
    def test_neutral_recommendation(self, mock_find_similar):
        ...
        self.assertEqual(response.status_code, 200)
        recommendation = response.context["recommendation"]
        self.assertEqual(recommendation.level, "neutral")
        self.assertIn("Borderline", recommendation.headline)
    ...
    def test_insufficient_data_no_recommendation(self, mock_find_similar):
        ...
        self.assertContains(response, "Not Enough Data for Recommendation")
...
class ProtestAnalysisViewTests(TestCase):
    ...
    def test_200_and_required_context_keys_present(self, mock_find):
        ...
        for attr in [
            ...
            self.assertTrue(hasattr(equity, attr), f"Missing equity attribute: {attr}")
    ...
    def test_equity_gap_and_savings_computed_correctly(self, mock_find):
        ...
        self.assertAlmostEqual(equity.subject_value_per_sqft, 185.0, places=1)
        self.assertAlmostEqual(equity.median_comp_value_per_sqft, 160.0, places=1)
        self.assertAlmostEqual(equity.equity_gap_per_sqft, 25.0, places=1)
        self.assertAlmostEqual(equity.estimated_savings, 50000.0, places=0)
    ...
    def test_comps_below_subject_counted_correctly(self, mock_find):
        ...
        mock_find.return_value = [self._similar_result(self.comp, self.comp_building)]
        response = self.client.get(reverse("protest_analysis", args=[self.target.account_number]))
        self.assertEqual(response.context["equity"].comps_below_subject, 1)
    ...
    def test_protest_analysis_hides_history_when_absent(self, mock_find):
        ...
        self.assertEqual(response.context["assessment_history"], [])
        self.assertIsNone(response.context["assessment_history_chart"])
        self.assertNotContains(response, "Cap Status")
