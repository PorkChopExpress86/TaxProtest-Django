"""Shapefile selection for the ``load_gis_data`` command.

The loader itself (``etl_pipeline/gis_loader.load_gis_parcels``) is covered by
``counties/harris/etl_pipeline/tests/test_ported_from_legacy.py``, next to the
code — including the centroid/CRS ordering that used to be asserted here.
"""
