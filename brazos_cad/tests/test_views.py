"""Tests for the Brazos County search view (brazos_cad/views.py)."""
...
from brazos_cad.models import PropertyAccount, PropertyLand
...
class BrazosIndexViewTests(TestCase):
    ...
    def test_account_without_coordinates_has_no_location(self):
        response = self.client.get(reverse("brazos_index"), {"owner_name": "SMITH"})
        results = response.context["results"]
        self.assertFalse(results[0]["has_location"])
