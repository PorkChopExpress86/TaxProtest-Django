...
export; see ``brazos_cad/parsers/pacs.py`` for the fixed-width field layout):
  ...
  - APPRAISAL_ENTITY_INFO.TXT        -> data.models.PropertyJurisdictionExemption
...
class PropertyBuildingCharacteristic(models.Model):
    ...
    (see brazos_cad/parsers/pacs.py) into one row per improvement -- wide/
    column-shaped, matching Harris's data.models.BuildingDetail, since the
...
class PropertyExtraFeature(models.Model):
    ...
    resolve, mirroring how data.models.ExtraFeature is Harris's own
