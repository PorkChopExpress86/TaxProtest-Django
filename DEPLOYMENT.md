...
4. Classifies each commit as one of:
   - **FULL** — any change to `docker-compose*.yml`, `Dockerfile`,
     ...
     `data/models.py`, `brazos_cad/models.py`, templates, or static
     files. Runs `docker compose up -d --build`.
   - **PARTIAL** — only BCAD/ETL-only code changed (`brazos_cad/management/`,
     `brazos_cad/parsers/`, `data/etl_pipeline/`, `data/etl.py`).
